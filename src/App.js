...
import Eire from "./contracts/Eire.json";
import MyTokenSale from "./contracts/MyTokenSale.json";
import KcyContract from "./contracts/KycContract.json";
  ...
  componentDidMount = async () => {
    try {
      ...
      this.tokenInstance = new this.web3.eth.Contract(
        Eire.abi,
        Eire.networks[this.networkId] && Eire.networks[this.networkId].address,
      );
      ...
      this.kycInstance = new this.web3.eth.Contract(
        KcyContract.abi,
        KcyContract.networks[this.networkId] && KcyContract.networks[this.networkId].address,
      );
...
handleKycWhitelisting = async () => {
  ...
  render() {
    ...
    return (
      <div className="App">
        <h1>Eire Token Sale</h1>
        <p>Get them straight from the mint, today!</p>
        <h2>KYC Whitelisting</h2>
        Address to allow: <input type="text" name="kycAddress" value={this.state.kycAddress} onChange={this.handleInputChange}/>
        <button type="button" onClick={this.handleKycWhitelisting}>Add to Whitelist</button>
        <h2>Buy Eire Tokens</h2>
        <p>If you want to buy tokens, send Wei to this address: {this.state.tokenSaleAddress}</p>
        <p>You currently have: {this.state.userTokens} EIR Tokens</p>
